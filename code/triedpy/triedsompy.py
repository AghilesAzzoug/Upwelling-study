...
import matplotlib
import pandas as pd
from   triedpy.triedtools import klavier #CS?
...
class SOM(object):
    ...
    #set SOM topology
    def set_topology(self, mapsize=None, mapshape='planar', \
                     lattice='rect', mask=None, compname=None):
        """
    ...
    def view_map(self, what='codebook', which_dim ='all', pack='Yes', \
        ...
        if np.min(mapsize) > 1 :
            if pack == 'No':
                ...
                #print('hi'); 
                view_2d_Pack(self, text_size, which_dim=which_dim, what=what, save=save, \
                             save_dir=save_dir, grid=grid,text=text, CMAP=cmap,col_sz=COL_SiZe)        
    ...
    def train(self, etape1=[5,5,2],etape2=[5,2,0.1], verbose='on', retqerrflg=False):
        ...
        3.   []         []      : DEUX etapes par defaut
        4.   []          0      : Que la 1ERE étape par defaut
        5.    0         []      : Que la 2EME étape par defaut
        6.   [X]        []      : 1ERE étape choisie ; 2EME etape par defaut
        7.   []         [X]     : 1ERE étape par defaur; 2EME étape choisie
             Quand on veut maitriser une étape, il va de soi qu'il faut
             renseigner les paramètres concernés '''
        t0      = time()
        ...
        if etape1 != 0 :
            ...
            #batchtrain(self, phase='rough', verbose=verbose)
            batchtrain(self, etape1, phase='rough', verbose=verbose)
            if verbose=='on':
                print()
        ...
        if etape2 != 0 :
            ...
            #batchtrain(self, phase='finetune',verbose=verbose)
            batchtrain(self, etape2, phase='finetune',verbose=verbose)
...
#               print
    ...
    def cluster(self,method='Kmeans',n_clusters=8):
        if n_clusters > self.mapsize[0]*self.mapsize[1] :   #CS
            n_clusters = self.mapsize[0]*self.mapsize[1]    #CS 
    ...
    def hit_map(self,data=None):
        ...
        ###############################
        #CS fig = plt.figure(figsize=(msz[1]/5,msz[0]/5))
        fig = plt.figure(); #CS
        ...
        plt.scatter(coord[:,1], msz[0]-.5 -coord[:,0], s=area.flatten(), alpha=.9,\
                    c='None',marker='o',cmap='jet',linewidths=3, edgecolor='r')
        plt.xlim(0,msz[1])
        plt.ylim(0,msz[0])
        plt.axis("tight"); #CS
...
#     	    plt.scatter(X, msz[0]-1-Y, s=area, alpha=0.9,c='None',marker='o',cmap='jet',linewidths=3, edgecolor = 'r')    		
    ...
    def view_U_matrix(self,distance2=1,row_normalized='No',show_data='Yes',contooor='Yes',blob = 'No',save='No',save_dir = '',fignum = None):
        ...
        #CS ratio = float(msz[0])/(msz[0]+msz[1])
        #CS fig.set_size_inches((1-ratio)*15,ratio*15)
        #en taguant ces 2 lignes ca ne plante plus la U-Matrice
        #pour une carte 25 1 de la lettre F
        plt.tight_layout()
        ...
        plt.suptitle("THE U-MATRICE"); #CS
        return sel_points, umat


    #CS def hit_map_cluster_number(self,data=None,interp='on'):
    def hit_map_cluster_number(self,data=None,interp='on') :
        ...
        msz =  getattr(self, 'mapsize')
        #CS fig = plt.figure(figsize=(msz[1]/2.5,msz[0]/2.5))
        fig = plt.figure(); #CS
        ax = fig.add_subplot(111)
        #ax.xaxis.set_ticklabels([])
        #ax.yaxis.set_ticklabels([])
        ...
        #CS plt.imshow(codebook.reshape(msz[0],msz[1])[::],alpha=.5);
        if interp == 'on' :
            plt.imshow(codebook.reshape(msz[0],msz[1])[::],alpha=.5);
        else :
            plt.imshow(codebook.reshape(msz[0],msz[1])[::],alpha=.5,interpolation=interp);     
    ...
    def update_codebook_voronoi(self, training_data, bmu, H, radius):
        ...
        row = inds
        col = np.arange(dlen)
        val = np.tile(1, dlen)      # repmat ?
        P   = csr_matrix( (val,(row,col)), shape=(nnodes,dlen) ) #scipy.sparse.csr_matrix : Compressed Sparse Row matrix
...
def batchtrain(self, etape, phase=None, verbose='on'):
    t0      = time()
    nnodes  = getattr(self, 'nnodes')
    dlen    = getattr(self, 'dlen')
    dim     = getattr(self, 'dim')
    ...
    #print("#CS mpd=%f, trainlen=%d" % (mpd,int(np.ceil(30*mpd)))); 
...
#           radiusfin = max(1, radiusin/2.)
    ...
    if etape != [] : # j'oublie tout ce qui a ete dit avant
        trainlen  = etape[0]; 
        radiusin  = etape[1];
        radiusfin = etape[2];           
...
#           print H
        ...
        bmu = self.para_bmu_find(data, New_Codebook_V)
        #CS if verbose=='on':
            #CS print()
...
def view_2d_Pack(self, text_size, which_dim='all', what='codebook', save='No', \
                 grid='Yes', save_dir='empty',text='Yes', CMAP='None', col_sz=None):
    import matplotlib.cm as cm
    msz0, msz1 = getattr(self, 'mapsize')
    ...
    if what == 'codebook':
        ...
        if which_dim == 'all':
            ...
            ratio     = np.max((.35,ratio))
            sH, sV    = 16,16*ratio*1
            #plt.figure(figsize=(sH,sV)); plt.show(); return
        elif type(which_dim) == int: 
            dim          = 1
            indtoshow    = np.zeros(1)
            indtoshow[0] = int(which_dim)
        ...
        while axisNum <dim:
            ...
            ind = int(indtoshow[axisNum-1]);           #cs : indice de variable
            mp  = codebook[:,ind].reshape(msz0, msz1); # les coordonnées des codebook coorespondant à la variable
        ...
        #return
        #cs figure suplementaire pour cluster !!!???
        if what == 'cluster' :
            if hasattr(self, 'cluster_labels'):
                codebook = getattr(self, 'cluster_labels');
            else :
        ...
        #CS mp = codebook[:].reshape(msz0, msz1)
        mp = codebook[:,0].reshape(msz0, msz1)
        if grid=='Yes':
...
def lininit(self):
    ...
    if np.min(msize)>1:
        ...
        #CS : un Warning indique d'utiliser PCA(svd_solver='randomized') a la place de
        #     RandomizedPCA, mais les résultats ne sont pas les memes et svd_solver semble
        ...
        #print(nnodes, eigvec.shape, codebook.shape, coord.shape)
        #        49      (4, 4)         (49, 4)      (49, 2)
